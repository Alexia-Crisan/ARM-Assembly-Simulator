...
from .pseudo_instruction_encoder import encode_pseudo_instruction
...
def encode_instruction(line: str, current_place: int, labels: Dict[str, int]) -> int:
    ...
    if instruction in _PSEUDO:
        return encode_pseudo_instruction(instruction, args, current_place, labels)
    
    if instruction in OPCODES:
        return encode_data_processing_instruction(instruction, args)
    
    if instruction in ("LDR", "STR", "LDRB", "STRB"):
    ...
    if instruction.startswith("B") or instruction == "JMS":  # any branch
